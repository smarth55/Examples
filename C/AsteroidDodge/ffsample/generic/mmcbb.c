...
#include <device.h>				/* Include device specific declareations here */

#define	INIT_PORT()	{ init_port(); }	/* Initialize control port (CS=H, CLK=L, DI=H, DO=pu) */
#define DLY_US(n)	{ dly_us(n); }		/* Delay n microseconds */

#define	CS_H()		bset(P0)	/* Set MMC CS 'H' */
#define CS_L()		bclr(P0)	/* Set MMC CS 'L' */
#define CK_H()		bset(P1)	/* Set MMC SCLK 'H' */
#define	CK_L()		bclr(P1)	/* Set MMC SCLK 'L' */
#define DI_H()		bset(P2)	/* Set MMC DI 'H' */
#define DI_L()		bclr(P2)	/* Set MMC DI 'L' */
#define DO			btest(P3)	/* Test for MMC DO ('H':true, 'L':false) */
