/*------------------------------------------------------------------------*/
/* FRK-RX62N: MMCv3/SDv1/SDv2 (SPIモード) 制御モジュール                   */
/*------------------------------------------------------------------------*/
/*
...
#define	CS_LOW()	{PORT6.DR.BIT.B5 = 0;}	/* CS#をLにする */
#define	CS_HIGH()	{PORT6.DR.BIT.B5 = 1;}	/* CS#をHにする */
#define	INS			(!PORT6.PORT.BIT.B6)	/* カード検出(真:あり, 偽:なし) */
#define	WP			0 //(PORT6.PORT.BIT.B7)	/* 書き込み禁止(真:禁止, 偽:許可) */
#define CS_INIT()	{	/* CS#,INS#,WP 各端子の初期化 */\
	PORT6.DR.BIT.B5 = 1; /* CS#=OUT */	\
	PORT6.DDR.BIT.B5 = 1; /* CS#=H */	\
	PORT6.ICR.BIT.B6 = 1; /* INS#=IN */	\
	PORT6.ICR.BIT.B7 = 1; /* WP=IN */	\
}
...
)
{
	...
	RSPI.SPCMD0.BIT.SPB = 3;	/* 32-bit mode */

	do {
		RSPI.SPDR.LONG = LDDW(*lp++);	/* Send four bytes */
		while (!RSPI.SPSR.BIT.SPRF) ;	/* Wait for end of transfer */
		RSPI.SPDR.LONG;					/* Discard received bytes */
	} while (btx -= 4);					/* Repeat until all data sent */

	RSPI.SPCMD0.BIT.SPB = 7;	/* 8-bit mode */
}
