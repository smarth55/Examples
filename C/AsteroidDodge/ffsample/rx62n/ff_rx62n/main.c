...
/* Console: N81 38400bps (port is defined in scif.c)                    */
...
void timer_init (void)
{
	...
	SYSTEM.SCKCR.LONG = 0x01020100;		/* Select system clock: ICLK=8x(96MHz), PCLK=4x(48MHz) */

	/* Initialize CMT0 (1kHz IVT) */
	MSTP_CMT0 = 0;						/* Enable CMT0 module */
	CMT0.CMCR.WORD = 0x0040;			/* CMIE=1, CKS=0(PCLK/8) */
	CMT0.CMCOR = PCLK / 8 / 1000 - 1;	/* Select clock divider */
	CMT.CMSTR0.BIT.STR0 = 1;			/* Start CMT0 */
	IPR(CMT0, CMI0) = 8;				/* Interrupt level = 8 */
...
int main (void)
{
	...
	timer_init();			/* Initialize clock and timer */
	scif_init();			/* Initialize serial port */
