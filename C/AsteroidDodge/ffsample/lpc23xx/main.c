...
IMPORT_BIN("bigsight.bin", BitMap1);	/* Built-in picture */
...
static
int nand_progress (	/* Call-back function for NAND_FORMAT ioctl command */
	WORD c_blk,		/* Processed blocks */
	WORD t_blk,		/* Total blocks to be processed */
	WORD t_bad		/* Total bad blocks */
)
{
...
int main (void)
{
	...
	for (;;) {
		...
		case 'g' :	/* Graphic controls */
			...
			case 'l' :	/* gl <x> <y> <color> - Draw line */
				if (!xatoi(&ptr, &p1) || !xatoi(&ptr, &p2) || !xatoi(&ptr, &p3)) break;
				disp_lineto(p1, p2, p3);
				break;

			case 'p' :	/* gp <x> <y> <color> - Set point */
				if (!xatoi(&ptr, &p1) || !xatoi(&ptr, &p2) || !xatoi(&ptr, &p3)) break;
				disp_pset(p1, p2, p3);
				break;
		...
		case 'd' :	/* Disk I/O layer controls */
			...
			case 'c' :	/* Disk ioctl */
				...
				case 'e' :	/* dce <drv> <start> <end> - CTRL_ERASE_SECTOR */
					...
					break;
				case 'f' :	/* dcf <drv> - NAND_FORMAT */
					if (!xatoi(&ptr, &p1)) break;
					xprintf("\nrc=%d\n", disk_ioctl((BYTE)p1, NAND_FORMAT, nand_progress));
					break;
				}
				break;
			}
