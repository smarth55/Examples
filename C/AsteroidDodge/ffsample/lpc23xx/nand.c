...
/* Fast memcpy functions (defined in asmfunc.S) */
...
int flush_block (void)
{
	...
	if (BlockDirty) {
		...
		//	xprintf("[W:%d]", pb);
			if (write_sector(SectorBuff[0], (DWORD)pb * N_SIZE_BLK, N_SIZE_BLK)) break;	/* Write-back the block */
...
)
{
	...
	while (sc) {
		...
		do {
			...
			if ((SectorBuff[i][130] >> 24) == 0xFF) {	/* Blank sector */
				for (j = 0; j < 512; j++)	/* Fill app buffer with 0xFF */
					buff[j] = 0xFF;
			} else {	/* Live sector (copy data) */
				if (!SectStat[i]) {	/* Check ECC if not checked */
				...
				Copy_al2un(buff, SectorBuff[i], 512);	/* Copy sector data to app buffer */
			}
			buff += 512; lba++; sc--;
...
)
{
	...
	while (sc) {
		...
		do {
			...
			SectorBuff[i][130] = create_ecc(&SectorBuff[i][0]);		/* Create ECC 1 (the sector goes live) */
